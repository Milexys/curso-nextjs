...
export default function Header() {
  ...
  return (
    <>
      <Disclosure as="nav" className={`bg-gray-800 ${!auth?.user && 'hidden'}`}>
        {({ open }) => (
          <>
                  ...
                  <div className="hidden md:block">
                    <div className="ml-10 flex items-baseline space-x-4">
                      {navigation.map((item) => (
                        ...
                        >
                          <a className={classNames(item.current ? 'bg-gray-900 text-white' : 'text-gray-300 hover:bg-gray-700 hover:text-white', 'px-3 py-2 rounded-md text-sm font-medium')}>{item.name}</a>
                        </Link>
                      ))}
                    </div>
                  </div>
